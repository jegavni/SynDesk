import type { AuthUser, CallLog, SidebarUser } from '../../types';
...
const CallLogList = ({ callLogs, authUser, isLoading, onInitiateCall }: CallLogListProps) => {
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button
                onClick={() => {
                  ...
                  const targetUser = { _id: peerUser._id, username: peerUser.username, profilePic: peerUser.profilePic, isGroup: false } as SidebarUser;
                  onInitiateCall(targetUser, 'voice');
                }}
                disabled={isCallActive}
                style={{
                  background: isCallActive ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.05)',
                  color: isCallActive ? 'rgba(255,255,255,0.2)' : 'white',
                  padding: '0.4rem',
                  borderRadius: '50%',
                  width: '32px',
                  height: '32px',
              ...
              <button
                onClick={() => {
                  if (isCallActive) return;
                  const targetUser = { _id: peerUser._id, username: peerUser.username, profilePic: peerUser.profilePic, isGroup: false } as SidebarUser;
                  onInitiateCall(targetUser, 'video');
                }}
                disabled={isCallActive}
                style={{
                  background: isCallActive ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.05)',
